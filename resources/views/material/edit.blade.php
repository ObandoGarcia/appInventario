        ...
        <div class="row">
            <div class="col-6">
                <form action="{{ route('actualizar_material', $material->id) }}" method="POST">
                    ...
                    <input type="number" step="1" name="cantidad" class="form-control" value="{{ $material->cantidad }}">
                    @error('cantidad')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
                    ...
                    <input type="datetime-local" name="fecha_de_ingreso" class="form-control" value="{{ $material->fecha_de_ingreso }}">
                    @error('fecha_de_ingreso')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror

                    <label for="precio_por_unidad">Precio por unidad</label>
                    <input type="number" step="0.01" name="precio_por_unidad" class="form-control" value="{{ $material->precio_por_unidad }}">
